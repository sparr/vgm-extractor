...
def extract(data):
    ...
    accumulate_outputs.stdout = archive_list_output.stdout
    accumulate_outputs.stderr = archive_list_output.stderr
    accumulate_outputs.returncode = max(
        archive_list_output.returncode, accumulate_outputs.returncode
    )
    file_list = [
